<div class="hidden h-96 w-1/2  flex-none flex-col @if (! is_null($user)) divide-y divide-gray-100 @else text-center @endif overflow-y-auto sm:flex">
    @if (isset($user['email']))
        <div class="flex-none p-6 text-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto text-gray-500 h-16 w-16 rounded-full" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="flex flex-auto flex-col justify-between p-6">
            <dl class="grid grid-cols-1 gap-x-6 gap-y-3 text-sm text-gray-700">
                ...
                <a type="button" 
                    ...
                    @endif
                    class="mt-6 w-full rounded-md border border-transparent bg-sky-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-sky-700 focus:outline-none text-center focus:ring-2 focus:ring-sky-500 focus:ring-offset-2"> <span class="mx-auto">View profile</span> </a>
        </div>
    @elseif(! is_null($user))
    <div class="my-auto ">
        <div class="flex-none p-6 text-center">
